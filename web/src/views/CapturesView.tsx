...
export function CapturesView({ api, config, onError, onSuccess }: Props) {
  const { t } = useTranslation(["captures", "common"]);
  ...
  return <div className="capture-shell">
    <section className="content-section"><div className="section-heading captures-heading"><div><h2>{t("captures:controlTitle")}</h2><p>{status?.active ? t("captures:activeSummary", { count: status.remainingRequests }) : t("captures:idleSummary")}</p></div><div className="header-actions">
      ...
      <div className="capture-controls">
        ...
        <div className="capture-usage"><span>{t("captures:storage")}</span><strong>{formatBytes(status?.storageBytes || 0)} / {formatBytes(status?.maxTotalBytes || 0)}</strong><i><b style={{ width: `${usage}%` }} /></i></div>
      </div>
    </section>
    <section className="content-section spaced"><div className="section-heading"><div><h2>{t("captures:recordsTitle")}</h2><p>{t("captures:recordsSummary", { count: records.length })}</p></div></div>
      {records.length === 0 ? <div className="empty-state"><FileLock2 />{t("captures:empty")}</div> : <div className="table-wrap capture-table"><table><thead><tr><th>{t("captures:request")}</th><th>{t("captures:state")}</th><th>{t("captures:attempts")}</th><th>{t("captures:size")}</th><th>{t("captures:expires")}</th><th /></tr></thead><tbody>
        {records.map((record) => <tr key={record.id}><td><strong>{record.method} {record.path}</strong><code className="subtle">{record.requestId}</code></td><td><span className={`status ${record.state}`}>{record.state}</span></td><td>{record.attempts.length}</td><td>{formatBytes(record.capturedBytes)}</td><td>{new Date(record.expiresAt).toLocaleString()}</td><td><div className="row-actions">
    ...
    {preview && <div className="drawer-backdrop" onMouseDown={() => setPreview(null)}><aside className="capture-drawer" onMouseDown={(e) => e.stopPropagation()}><div className="drawer-header"><div><span className="status">{preview.record.state}</span><h2>{preview.record.method} {preview.record.path}</h2><p>{preview.record.requestId}</p></div><button className="icon-button" aria-label={t("common:actions.close")} onClick={() => setPreview(null)}><X size={18} /></button></div>
      <div className="capture-parts">{preview.parts.map((part, index) => <section key={`${part.name}-${part.attempt || index}`}><div><strong>{part.name === "attempt" ? t("captures:attemptPart", { count: part.attempt }) : t(`captures:${part.name}Part`)}</strong><span>{formatBytes(part.originalBytes)}{part.truncated ? ` · ${t("captures:truncated")}` : ""}</span></div><details><summary>Headers</summary><pre>{JSON.stringify(part.headers || {}, null, 2)}</pre></details><pre className="capture-body">{part.body}</pre></section>)}</div>
    </aside></div>}
  </div>;
}
