...
export function CapturesView({ api, config, onError, onSuccess, canOperate, canSensitive, selectedId, confirm }: Props) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setStatus(nextStatus); setRecords(nextRecords); setKeys(nextKeys);
  ...
  async function rewrapKeys() {
    ...
    try {
      ...
      onSuccess(t("captures:keys.rewrapped", { count: result.updated })); await load();
    } catch (reason) { onError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setBusy(false); }
  }
    ...
    <section className="content-section spaced"><div className="section-heading"><div><h2>{t("captures:recordsTitle")}</h2><p>{t("captures:recordsSummary", { count: records.length })}</p></div></div>
      {records.length === 0 ? <div className="empty-state"><FileLock2 />{t("captures:empty")}</div> : <div className="table-wrap capture-table responsive-table"><table><thead><tr><th>{t("captures:request")}</th><th>{t("captures:state")}</th><th>{t("captures:attempts")}</th><th>{t("captures:size")}</th><th>{t("captures:expires")}</th><th /></tr></thead><tbody>
        {records.map((record) => <tr key={record.id}><td data-label={t("captures:request")}><strong>{record.method} {record.path}</strong><code className="subtle">{record.requestId}</code></td><td data-label={t("captures:state")}><span className={`status ${record.state}`}>{t(`common:status.${record.state}`, { defaultValue: record.state })}</span></td><td data-label={t("captures:attempts")}>{record.attempts.length}</td><td data-label={t("captures:size")}>{formatBytes(record.capturedBytes)}</td><td data-label={t("captures:expires")}>{new Date(record.expiresAt).toLocaleString(i18n.resolvedLanguage)}</td><td data-label={t("captures:actions")}><div className="row-actions">
          ...
          {canSensitive && <button className="icon-button sensitive" data-tooltip={t("captures:rawDownload")} aria-label={t("captures:rawDownload")} onClick={() => download(record, "raw")}><FileLock2 size={16} /></button>}
          {canOperate && <button className="icon-button danger" data-tooltip={t("captures:delete")} aria-label={t("captures:delete")} onClick={() => remove(record)}><Trash2 size={16} /></button>}
        </div></td></tr>)}
      </tbody></table></div>}
    </section>
    <section className="content-section spaced"><div className="section-heading"><div><h2>{t("captures:keys.title")}</h2><p>{t("captures:keys.summary")}</p></div>{canOperate && <button className="button" disabled={busy || !keys} onClick={rewrapKeys}><FileLock2 size={16} />{t("captures:keys.rewrap")}</button>}</div>
      {!keys ? <div className="empty-state compact">{t("common:loading")}</div> : <div className="capture-key-grid">
        <div><span>{t("captures:keys.active")}</span><strong>{keys.activeId}</strong></div>
        <div><span>{t("captures:keys.configured")}</span><strong>{keys.configured.join(", ") || t("common:notAvailable")}</strong></div>
