...
export function LogsView({ api, onError }: Props) {
  ...
  return <section className="content-section">
    ...
    <div className="log-filters">
      <label className="field"><span>{t("logs:level")}</span><select value={level} onChange={(e) => setLevel(e.target.value)}><option value="">{t("logs:all")}</option><option value="info">Info</option><option value="warn">Warn</option><option value="error">Error</option></select></label>
      <label className="field"><span>{t("logs:event")}</span><input value={event} onChange={(e) => setEvent(e.target.value)} placeholder="upstream.request_failed" /></label>
    ...
    {entries.length === 0 ? <div className="empty-state"><ScrollText />{t("logs:empty")}</div> : <div className="runtime-log" aria-live="polite">
      {entries.map((entry) => <article className={`log-entry ${entry.level}`} key={entry.id}>
        <time>{new Date(entry.time).toLocaleTimeString()}</time><span className="log-level">{entry.level}</span><div><strong>{entry.event}</strong><p>{entry.message}</p>{entry.requestId && <code>{entry.requestId}{entry.attempt ? ` · #${entry.attempt}` : ""}{entry.statusCode ? ` · HTTP ${entry.statusCode}` : ""}</code>}</div>
